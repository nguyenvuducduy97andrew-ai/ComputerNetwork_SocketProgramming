...
python -u -m server.main_server
```

Server mặc định lắng nghe TCP trên `0.0.0.0:2121` và sử dụng thư mục `data/server_storage/` làm server root.

Terminal 2 (client):
```powershell
cd 'c:\Users\PC\OneDrive\Máy tính\MMT_Projects_SocketPrograming'
python -u -m client.main_client --host localhost --port 2121
```
